...
Explorando os IDs, encontrei tráfego de rede presente no id "0", baixei o arquivo 0.pcap disponível e fui analisar no wireshark. Analisando o tráfego e troca de pacotes, lá estava presente uma conexão FTP que diponilibizava o User e o password do usuário "nathan".
...
Com essas credenciais, consegui entrar no serviço ftp com o usário "nathan".Dentro do serviço, consegui a primeira flag do ctf "user.txt" e consegui um arquivo linpeas.sh, que baixei para a minha máquina.  
...
Também com as credenciais do usuário "nathan", consegui me conectar na máquina alvo via SSH. Já conectado ao sistema como usuário comum, eu precisava do arquivo limpeas.sh nesta máquina, então abri um server no meu sistema que permitia outras máquinas baixarem os arquivos na pasta presente.  
...
Executei o linpeas na máquina alvo e ele me revelou que o python3 tinha cap_setuid, que é a capacidade de alterar o id dos usuários, oq me possibilitaria alterar o id do usuário "nathan" o tornando root ( root = id 0).
...
## Conclusão
A máquina Cap explora uma falha de IDOR  em uma aplicação web, onde a manipulação de identificadores numéricos permitiu o acesso a dados de outros usuários. Por meio dessa vulnerabilidade, foi possível obter informações sensíveis e credenciais que possibilitaram o acesso ao sistema via SSH, culminando na leitura da flag.
